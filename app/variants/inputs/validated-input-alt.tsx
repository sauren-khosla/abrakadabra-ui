    ...
    <div className="w-full max-w-md">
      ...
      {status && (
        <div className="mt-2 flex items-center space-x-1 text-xs">
          ...
          {status === "checking" && <span className="text-yellow-500">Checking availability...</span>}
          {status === "available" && <span className="text-green-500">Username is available!</span>}
          {status === "unavailable" && <span className="text-red-500">Username is unavailable.</span>}
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col justify-between items-center w-full h-full">
      ...
      <div className="text-xs text-gray-500 pb-2">
        The unavailable usernames are <code>john</code>, <code>jane</code>, <code>admin</code>, and <code>user123</code>.
      </div>
    </div>
  )
}